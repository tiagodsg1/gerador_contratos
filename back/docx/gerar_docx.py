...
class GerarDocx:
    def __init__(self, t_contrato, caminho_documento, dicionario):
        ...
        if self.t_contrato == 'Consultoria':
            ...
            self.sucesso = dicionario['sucesso']
            self.error = dicionario['error']
            consultoria(self.dados_cliente, self.dados_corretor, self.dados_imovel, self.min_valor, self.av_valor, self.pro_valor, self.cons_valor, self.caminho_documento, self.sucesso, self.error)